...
//*********SIMULATION METHODS
//Setting simulation studio for calls coming in
Server.prototype.setSimStudio = function(id){
    if(id && typeof this.studios[id] !== 'undefined'){
        this.simStudio = id;
    }else{
        this.simStudio = 1;
    }
...
Server.prototype.simulateCall = function(seedId){
    if(seedId) var caller = random.createCaller(seedId);
    else var caller = random.createCaller();
    //Studio will take care of handling which line to place the call on
    var status = this.studios[this.simStudio].createCall(caller);
    caller.success = status.success;
    caller.lineId = status.lineId;
    if(status.success){
        var msg = "event studio.line#"+status.lineId+" "+status.msg;
        this.broadcast(this.simStudio, msg);
    }
    return caller;
}
//Simulate up the studio handset
Server.prototype.simulatePickup = function(lineId){
    this.studios[this.simStudio].putOnHandset(lineId);
}
//Simulate caller hanging up
Server.prototype.simulateHangup = function(callId){
    this.studios[this.simStudio].hangupLine(callId);
}
...
Server.prototype.studioLineHandler = function(id, body){
    var objectId = "studio.line#" + body.subObjectId;
    var clientStudioId = this.clients[id].getStudio();
    //Verify selected studio
    if(clientStudioId == 0 || clientStudioId == null) return this.error(id, objectId, 'UNSELECTED_STUDIO');
    //Get number of lines for chosen studio
    var numberOfLines = this.studios[clientStudioId].getNumberLines();
    //Try to produce a line number from the sub-object id
    try{
        var lineNumber = parseInt(body.subObjectId);
    }catch(e){
        var lineNumber = 0;
    }
    //Check for valid line number based on chosen studio
    if(lineNumber < 1 || lineNumber > numberOfLines) return this.error(id, objectId, null, "Nonexisting line '" + body.subObjectId + "'");
    var op = body.operation;
    var props = (body.properties !== null)? body.properties: [];
    var propNames = (body.properties !== null)? lwcp.propNames(body.properties) : [];
    if(op == 'get'){
        if(propNames.length == 0) return this.error(id, objectId, 'NO_VALID_PROPERTIES');
        var msg = this.studios[clientStudioId].getLineProps(lineNumber, propNames);
        if(!msg) return this.error(id, objectId, 'NO_VALID_PROPERTIES');
        return this.send(id, "indi " + objectId + " " + msg);
    }else if(op == 'set'){
        if(propNames.length == 0) return this.error(id, objectId, 'NO_VALID_PROPERTIES');
        var comment = lwcp.getPropValue('comment', props);
        if(typeof comment == 'string' && comment !== 'undefined'){
            var msg = "event " + objectId + " " + this.studios[clientStudioId].setComment(lineNumber, comment);
            return this.broadcast(clientStudioId, msg);
        }else{
            return this.error(id, objectId, 'NO_VALID_PROPERTIES');
        }
    }else if(op == 'seize'){
        return this.error(id, objectId, 'FEATURE');
    }else if(op == 'call'){
        return this.error(id, objectId, 'FEATURE');
    }else if(op == 'take'){
        if(propNames.length == 0){
            var msg = this.studios[clientStudioId].putCallOnAir(lineNumber, 5);
        }else if(propNames.length !== 0){
            var handset = lwcp.getPropValue('handset', props);
            if(typeof handset !== 'boolean') return this.error(id, objectId, 'NO_VALID_PROPERTIES');
            if(handset) var msg = this.studios[clientStudioId].putCallOnHandset(lineNumber);
            else{
                var hybrid = lwcp.getPropValue('hybrid', props);
                if(typeof hybrid !== 'number') hybrid = 5;
                this.studios[clientStudioId].putCallOnAir(lineNumber, hybrid);
            }
        }
    }else if(op == 'drop'){
        return this.error(id, objectId, 'FEATURE');
